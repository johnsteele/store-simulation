 ...
 *	- Allows getting the count of an Item.
 * 
 * Assumptions:
...
public:
	 ...
	 * @param count The occurence count of Item.
 	 */
	Item (char type, string name, string title, 
					int year, int count);
	 ...
	 * Preconditions: None.
	 * 
 	 * Postconditions: A pointer to a Item is returned.
	 * 
	...
	//---------------------getType--------------------------------
	/**
	 * @brief An accessor method for getting the type.
 	 *
...
private:
	...
	/**
	 * @brief The number of occurences of this Item.
	 */
	int my_count;  
};
#endif /* ITEM_H */
